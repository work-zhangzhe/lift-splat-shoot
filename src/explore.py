...
def viz_model_preds(
    ...
    model = compile_model(grid_conf, data_aug_conf, outC=1)
    print("loading", modelf)
    model.load_state_dict(torch.load(modelf, map_location="cpu"))
    model.to(device)
    ...
    with torch.no_grad():
        for batchi, (
            ...
            # imgs.shape = torch.Size([4, 6, 3, 128, 352])  # 图片
            # rots.shape = torch.Size([4, 6, 3, 3])         # 相机外参——旋转参数
            # trans.shape = torch.Size([4, 6, 3])           # 相机外参——平移参数
            # intrins.shape = torch.Size([4, 6, 3, 3])      # 相机内参
            # post_rots.shape = torch.Size([4, 6, 3, 3])    # 原图到模型输入尺寸的转换等比例缩小（缩小 0.22 倍）
            # post_trans.shape = torch.Size([4, 6, 3])      # 等比例缩小后的裁剪到模型输入尺寸（w平移-48）
            out = model(
                imgs.to(device),
                rots.to(device),
                trans.to(device),
